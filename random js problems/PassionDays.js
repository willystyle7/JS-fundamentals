function passionDays(input) {
    ...
    let money = new BigNumber(input.shift());
    // if (money.length > 16) {
    //     money.push(0);
    // }
    ...
    while (command !== 'mall.Exit') {
        for (let i = 0; i < command.length; i++) {
            ...
            if (action >= 'A'.charCodeAt(0) && action <= 'Z'.charCodeAt(0)) {
                ...
                action >= 'a'.charCodeAt(0) &&
                action <= 'z'.charCodeAt(0)
            ) {
                price = 0.3 * action;
            ...
            if (price > money || !(Math.round(money) > 0)) {
                continue;
            }
    ...
    if (purchases === 0) {
        console.log(`No purchases. Money left: ${money} lv.`);
    } else {
        console.log(
            `${purchases} purchases. Money left: ${money} lv.`
        );
    }
}
