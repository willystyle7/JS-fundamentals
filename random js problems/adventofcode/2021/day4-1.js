...
function solve(input) {
    ...
    return passports.map(p => readPassport(p)).filter(p => isValidPassport(p)).length;

    function readPassport(str) {
        let passport = {};
        let pairs = str.split(/\s+/g);
        pairs.forEach(pair => {
    ...
    function isValidPassport(passport) {
        let requiredFields = ['byr', 'iyr', 'eyr', 'hgt', 'hcl', 'ecl', 'pid']; // 'cid' is not required for passport
