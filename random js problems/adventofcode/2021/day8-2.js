...
function solve(input) {
    ...
    function findDigit(letters, mapping) {
        let number = '';
        for (let digit in mapping) {
            if (letters.length === mapping[digit].length && isArrayInArray(letters.split(''), mapping[digit].split(''))) {
                number = digit;
                break;
            }
        }
    ...
    function findMapping(nums10) {
        let mapping = {};
        for (let letters of nums10) {
            if (letters.length in uniqueLengths) {
                // 1, 4, 7, 8
                mapping[uniqueLengths[letters.length]] = letters;
            }
        }
        for (let letters of nums10) {
            if (letters.length === 5) {
                // 2, 3, 5
                if (isArrayInArray(mapping['7'].split(''), letters.split(''))) {
                    // 3
                    mapping['3'] = letters;
                } else if (inArray3el(mapping['4'].split(''), letters.split(''))) {
                    // 5
                    mapping['5'] = letters;
                } else {
                    // 2
                ...
                if (!isArrayInArray(mapping['7'].split(''), letters.split(''))) {
                    // 6
                    mapping['6'] = letters;
                } else if (isArrayInArray(mapping['4'].split(''), letters.split(''))) {
                    // 9
                    mapping['9'] = letters;
                } else {
                    // 0
    ...
    let sum = 0;
    for (let row of arr) {
        let mapping = findMapping(row.nums10);
        // console.log(mapping);
        // find number
        let number = '';
        for (let letters of row.nums4) {
            let digit = findDigit(letters, mapping);
            number += digit;
        }
...
console.log(solve(input1)); // 1007675
