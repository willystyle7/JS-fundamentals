...
function solve(input) {
    ...
    while (input.length > 0) {
        let [row1, row2, row3] = input.splice(0, 3);
        commons.push(findCommon(row1, row2, row3));
    }
    // return commons.map(findPriority);
    return commons.map(findPriority).reduce((a, b) => a + b, 0);
}

function findCommon(row1, row2, row3) {
    let common = null;
    for (const letter of row1) {
        if (row2.indexOf(letter) > -1 && row3.indexOf(letter) > -1) {
            common = letter;
            break;
        }
    }
