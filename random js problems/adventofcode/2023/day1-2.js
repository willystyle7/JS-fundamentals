...
function solve(input) {
    ...
    const rgx = /one|two|three|four|five|six|seven|eight|nine/;
    const rgxReverse = /eno|owt|eerht|ruof|evif|xis|neves|thgie|enin/;

    return input
        .split('\n')
        .map((ln) => {
            ln = ln.replace(rgx, (match) => `${mapping[match]}${match}`); // we preserve match when replace in cases "eighthree" is 83 and for "sevenine" is 79
            ln = ln.split('').reverse().join('');
            ln = ln.replace(rgxReverse, (match) => `${mappingReverse[match]}`);
            ln = ln.split('').reverse().join('');
            // console.log(ln);
            return ln;
        })
        .map((ln) => ln.split('').filter((ch) => /\d/.test(ch)))
        .map((ln) => Number(`${ln[0]}${ln[ln.length - 1]}`))
...
console.log(solve(input)); // 281

let input1 = `76xkqjzqtwonfour
...
console.log(solve(input1)); // 53348
