...
function solve(input) {
    ...
    for (let line of lines) {
        let [gameInfo, games] = line.split(':');
        let id = Number(gameInfo.match(/^Game (\d+)$/)[1]);
        games = games.split(';');
        let possible = true;
        for (let round of games) {
            let boxes = round.split(',');
            for (let set of boxes) {
                let matches = set.match(/^\s*(\d+)\s*([a-z]+)\s*$/);
                let num = Number(matches[1]);
                let color = matches[2];
                if (num > limits[color]) {
                    possible = false;
                    break;
...
console.log(solve(input)); // 8

let input1 = `Game 1: 9 red, 5 blue, 6 green; 6 red, 13 blue; 2 blue, 7 green, 5 red
