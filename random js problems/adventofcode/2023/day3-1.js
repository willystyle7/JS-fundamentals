...
function solve(input) {
    function checkNum(lines, row, col, len) {
        let left = Math.max(0, col - 1);
        let right = Math.min(lines[0].length - 1, col + len);
        ...
        for (let x = left; x <= right; x++) {
            if (isSymbol(lines[up][x])) {
                return true;
            }
            if (isSymbol(lines[down][x])) {
                return true;
            }
        }
        return false;
    ...
    for (let row = 0; row < lines.length; row++) {
        ...
        while ((match = rgx.exec(line)) !== null) {
            let num = match[0];
            let col = match.index;
            if (checkNum(lines, row, col, num.length)) {
                console.log(num);
                sum += Number(num);
            }
        }
...
console.log(solve(input1)); // 528819
