...
function solve(input) {
    function surroundNum(lines, stars, row, col, num) {
        let left = Math.max(0, col - 1);
        let right = Math.min(lines[0].length - 1, col + num.length);
        let up = Math.max(0, row - 1);
        let down = Math.min(lines.length - 1, row + 1);
        if (lines[row][left] === '*') {
            addStar(stars, row, left, num);
        }
        if (lines[row][right] === '*') {
            addStar(stars, row, right, num);
        }
        for (let x = left; x <= right; x++) {
            if (lines[up][x] === '*') {
                addStar(stars, up, x, num);
            }
            if (lines[down][x] === '*') {
                addStar(stars, down, x, num);
            }
        }
        return false;
    }

    function addStar(stars, row, col, num) {
        let coords = `${row}:${col}`;
        if (!(coords in stars)) {
            stars[coords] = [];
        }
        stars[coords].push(Number(num));
    }
    ...
    for (let row = 0; row < lines.length; row++) {
        ...
        while ((match = rgx.exec(line)) !== null) {
            let num = match[0];
            let col = match.index;
            surroundNum(lines, stars, row, col, num);
        }
    }

    let sum = 0;
    // console.log(stars);
    for (const coords in stars) {
        if (stars[coords].length === 2) {
            let gear = stars[coords][0] * stars[coords][1];
            sum += gear;
        }
    }
    return sum;
...
console.log(solve(input)); // 467835

let input1 = `.479........155..............944.....622..............31.........264.......................532..........................254.........528.....
..............-...............%.....+...................=....111*.................495.......+.......558..................../..........*.....
...
console.log(solve(input1)); // 80403602
